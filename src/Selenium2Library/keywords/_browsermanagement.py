...
FIREFOX_PROFILE_DIR = os.path.join(ROOT_DIR, 'resources', 'firefoxprofile')
FIREBUG_EXTENSION = os.path.join(FIREFOX_PROFILE_DIR, 'firebug-1.9.1-fx.xpi')
FIREPATH_EXTENSION = os.path.join(FIREFOX_PROFILE_DIR, 'firepath-0.9.7-fx.xpi')
...
class _BrowserManagementKeywords(KeywordGroup):
    ...
    def _make_browser(self, browser_name):
        ...
        if browser_token == '*firefox':
            #TODO: need to refactor so I can pass this in
            ...
            profile.set_preference("extensions.firebug.currentVersion", "1.9.1")            
