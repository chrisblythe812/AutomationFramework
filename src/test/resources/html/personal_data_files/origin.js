...
function prepareTextToShowCkeditor(text) {
 if (text && Object.isString(text)) {
     text = text.gsub("&amp;", "&"); //In case we have double & encoding
     text = text.gsub("&#59;", ";"); //In case we have double & encoding
     text = text.unescapeHTML();
     text = text.gsub("&#47;", "/");
     text = text.gsub("&#92;", "\\");
...
function wordWrap(value, maxLength){
	if (value.length > maxLength){
		...
		while(value.indexOf(" ") != -1 && value.length > maxLength){
		...
		var returnvalue = "";
			...
			returnvalue = value + "<br /> " + returnvalue;
			return returnvalue;	
	}
	else{
		return value;
...
processXmlIn: function(options) {
    ...
    if (!Object.isEmpty(gcc) && !Object.isEmpty(lcc)) {
        ...
        if (global.companies) {
            gcc = global.companies.values().first().gcc;
            lcc = global.companies.values().first().lcc;
            xmlJson.EWS.GCC = gcc;
            xmlJson.EWS.LCC = lcc;
        }
    }
