var Metadata = Class.create(Menu, {
    ...
    __MD_Config: function() {
        ...
        {'name': this.Metatada.type, 'label': global.getLabel('KM_TYPE_OF_CONTENT'), 'type': this.ItemType.dropdown, 'value': '', multi: true },
            ...
            handler: function(n) {
                ...
                switch (n) {
                    ...
                    case this.Metatada.businessRoles:
						...
						var v2 = this['__AC_CC_BUSINESS_ROLE_NUMBER'].getValue();
						var e1 = $(n + '_val');
                        var e2 = $(n + '_val_2');
                        if (e1 && e2 && (v1 || v2)) {
                            var vid = ((v1) ? v1.idAdded : v2.textAdded);
        ...
        if (params.length > 0) {
		    ...
		    '  <I_I_METADATA>';
                params.each(function(p) {
                    ...
                    if ((p.values.length == 0) && ((p.name == 'IS_TEMPLATE') || (p.name == 'ACCESS_BY_EXTERNAL'))) {
                        xmlin += '<YGLUI_STR_ECM_KM_META_VALV2 VALUE="" />';
                    }
    ...
    get_CC_CGROUP: function() {
        ...
        this.__doRequest('KM_GET_WC_TYPE', '__build_CC_CGROUP', []); 
    },
    ...
    get_CC_BUSINESS_ROLE: function() {
        ...
        this.__AC_CC_BUSINESS_ROLE_NUMBER = new JSONAutocompleter('AC_CC_BUSINESS_ROLE_NUMBER', { events: $H({}) }, js);
        this.__doRequest2('GET_RELAT_S_OBJ', '__build_CC_BUSINESS_ROLE');
    },
    ...
    __build_CC_BUSINESS_ROLE: function(json) {
        ...
        this.__AC_CC_BUSINESS_ROLE_NUMBER.updateInput(jsNumber);
    },
        ...
        items.each(function(i) {
            ...
            switch (f) {
                ...
                case 'CTYPE':
                    ...
                    break;
                case 'CGROUP':
                    var AC = this['__AC_CC_' + f];
                    var v = this.getMTVal(i);
                    if (AC && (v.length > 0)) {
                        try {
                            AC.setDefaultValue(v[0].val, false, false);
                        } catch (er) {
                            ;
                        }
                    }
