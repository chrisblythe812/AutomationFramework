 ...
 * <br/>Modified for 4.0
 * <ul>
 * <li>1052400 - If we are not in SCM, give the list of contextual actions of my pool</li>
 * </ul>
 * <br/>Modified for 4.0
 * <ul>
 * <li>1041193 - Add of the appId for ticket search in the case when determining current application</li>
 * </ul>
 * <br/>Modified for 4.2
 * <ul>
 * <li>Add of the appId for the dashboard in the case when determining current application</li>
 * </ul>
 */
var ScmUserAction = Class.create(/** @lends ScmUserAction.prototype */{
	...
	getCurrentAppId: function() {
		...
		switch(view) {
			...
			//since 4.0 - 1041193 - Add of the appId for ticket search in the case when determining current application
			case 'scm_searchTicket'		: view = 'OV_SEAR';	 	break;
			//since 4.2 - Add of the appId for the dashboard in the case when determining current application
    ...
    getListActions: function(event) {
        var cache;
		if(this._useCaching() === true) cache = ScmUserAction._cache.get(this._caller.appName + '_' + this._user.company+'_'+this._user.id);
        ...
        if(Object.isEmpty(cache))
            this._caller.makeAJAXrequest($H({
				xml				: 	'<EWS>'
								...
								+		'<OBJECT TYPE="P" TEXT="'+this._user.id+'"/>'
								+		'<PARAM>'
								+			'<CONTAINER>'+this.getCurrentAppId()+'</CONTAINER>'
