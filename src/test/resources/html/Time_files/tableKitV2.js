    ...
    **/
    _storingBackupData: function (table, enableFilter) {
        ...
        for (var i = 1; i <= arrayRows.length; i++) {
            ...
            for (var j = 0; j < this._columnHeader.length; j++) {
                if (arrayRows[i - 1].cells && arrayRows[i - 1].cells[j] && arrayRows[i - 1].cells[j].innerHTML)
                    var content = arrayRows[i - 1].cells[j].innerHTML.stripTags();
                else
                    var content = "";
                content = arrayRows[i - 1].cells ? this.exclusionExpandableContent(arrayRows[i - 1].cells[j], content) : content;
                ...
                if (!Object.isEmpty(this.truncateColumns)) {
                    ...
                    if (exists) {
                        this.truncateCell(arrayRows[i - 1].cells[j])
                    }
                }
            }
