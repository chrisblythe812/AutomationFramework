var megaButtonDisplayer = Class.create({
    ...
    createButton: function(button, index) {
        var aux = null;
		var title = Object.isEmpty(button.toolTip) ? prepareTextToEdit(button.label) : prepareTextToEdit(button.toolTip);
...
//                }
    ...
    updateLabel: function(idButton, label,updateToolTip) {
        ...
        if(updateToolTip){
            this.updateToolTip(idButton, label);
        }
    },
    /*
    * Method called when you want to update the link of any button. It can be called by updatelabel method or by the application itself.
    */
    updateToolTip: function(idButton, label){
        if (this.hash.get(idButton)[1].down('[class*=centerRoundedButton]'))
            this.hash.get(idButton)[1].down('[class*=centerRoundedButton]').title = label;
        else if (this.hash.get(idButton)[1].down('[class*=centerRoundedButtonDisable]'))
            this.hash.get(idButton)[1].down('[class*=centerRoundedButtonDisable]').title = label;
        else if ((!Object.isEmpty(this.hash.get(idButton)[0].className)) && (this.hash.get(idButton)[1].down('[class*=' + this.hash.get(idButton)[0].className + ']')))
            this.hash.get(idButton)[1].down('[class*=' + this.hash.get(idButton)[0].className + ']').title = label;
        else
            this.hash.get(idButton)[1].title = label;
    },
