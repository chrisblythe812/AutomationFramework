var ContentBrowser = Class.create(Application, {
    ...
    bulidUI: function() {
        ...
        if (this.__multiSelect) {
            $('km_actionsCont_incExp').observe('click', function() {
				if(!this.__searchMode){
					this.getFC(this.__slfo);
				}else{
					this.searchContentBrowser();
				}
            } .bind(this));
        }
    ...
    __buildActUI: function() {
       ...
       var json = ({
		...
		this.__searchBar = new Element('input',{'type': 'text','style': 'float:left;margin-right: 3px; margin-top: 2px;','size': '20','value': global.getLabel('DM_SEARCH')});		
		var clearSearchBar = ({
                        label: global.getLabel('Clear Search'),
                        idButton:'km_clearSearch',
                        className: (!global.liteVersion)?'application_action_link': '',
                        type: 'button',
						handler: this.clearSearchBar.bind(this),
                        standardButton: (!global.liteVersion)? true : false
        });
        json.elements.push(clearSearchBar);
		
		this.__secondaryButtons = new megaButtonDisplayer(json);
		var div = new Element ('div',{
				style:'float:right;margin-top:5px;width:100%;'
				}).insert(this.__secondaryButtons.getButtons());	
		
		this.__actionsCont.insert(this.__btns.getButtons());
		this.__actionsCont.insert(div);
		
		this.__secondaryButtons.disable('km_clearSearch');
		if(km_downloadLastExport){
			$('km_downloadLastExport').insert({'after': this.__searchBar});
		}
		this.__searchBar.observe('keyup', this.searchContentBrowser.bind(this));
		this.__searchBar.observe('blur', this.searchContentBrowserBlur.bind(this));
		this.__searchBar.observe('focus', this.searchContentBrowserFocus.bind(this));
    },
	
	clearSearchBar: function(){
		this.__searchMode = false;
		this.bulidUI();
		this.__secondaryButtons.disable('km_clearSearch');
		this.__searchBar.value = global.getLabel('DM_SEARCH')
	},
	
	searchContentBrowserFocus: function(){
		if(!this.__searchMode){
			this.__searchBar.value = '';
		}
	},
	
	searchContentBrowserBlur: function(){
		var searchText = this.__searchBar.value;
		if (searchText == '') {
			this.bulidUI();
			this.__secondaryButtons.disable('km_clearSearch');
			this.__searchMode = false;
	...
	searchContentBrowser: function(e){
	
		if ((e.keyCode == '13')&&(this.__searchBar.value)){
			this.__searchMode = true;
			...
			var searchText = this.__searchBar.value;
			if($('km_collapse_expand')){
				$('km_collapse_expand').hide();
			}
			if($('km_treeCont')){
				$('km_treeCont').hide();
			}
			this.__listCont.setStyle({ 'width': '100%' });
			this.__secondaryButtons.enable('km_clearSearch');
			
			var o = $('km_actionsCont_incExp');
			this.makeAJAXrequest($H({ xml:
				'<EWS>' +
					'<SERVICE>KM_SEARCH_BRWSR</SERVICE>' +
					'<PARAM>' +
					'<I_V_SEARCH_PATTERN>'+searchText+'</I_V_SEARCH_PATTERN>'+
					'<I_V_INCLUDE_EXPIRED>'+((o.checked)? 'X': '')+'</I_V_INCLUDE_EXPIRED>'+
					'</PARAM>' +
				 '</EWS>',
    ...
    importD: function() {
		...
		if(destFolder=='0'){
			...
			'<table style="width:100%;">' +
				...
				'<tr><td>' + global.getLabel('KM_IMPORT_OPTIONS') + ': </td></tr>' +
				'<tr><td>' + global.getLabel('KM_IMPORT_FORCEDRAFT') + ': </td><td><input type="checkbox" id="km_contBrowser_forceDraft" style="margin-left:-1px;"/></td></tr>' +
				'<tr><td>' + global.getLabel('KM_IMPORT_DELSOURCE') + ': </td><td><input type="checkbox" id="km_contBrowser_delSource" style="margin-left:-1px;"/></td></tr>' +
				'<tr><td>' + global.getLabel('KM_IMPORT_BYPASSMETA') + ': </td><td><input type="checkbox" id="km_contBrowser_byPass" style="margin-left:-1px;"/></td></tr>' +	
			'</table>' +
			'</div>' +
		...
		if(destFolder!='0'){		
			...
			var checkBoxForceDraft = $('km_contBrowser_forceDraft') || '';
			var checkBoxDelSource = $('km_contBrowser_delSource') || '';
			var checkBoxByPass = $('km_contBrowser_byPass') || '';

			if(checkBoxForceDraft){
				checkBoxForceDraft.observe('click', function() {
					if(checkBoxForceDraft.checked){
						this.__uploadModule.addParameter('I_V_FORCE_DRAFT','X');
					}else{
						this.__uploadModule.removeParameter('I_V_FORCE_DRAFT');
					}
				} .bind(this));
			}
			if(checkBoxDelSource){
				checkBoxDelSource.observe('click', function() {
					if(checkBoxDelSource.checked){
						this.__uploadModule.addParameter('I_V_DEL_SOURCE','X');
					}else{
						this.__uploadModule.removeParameter('I_V_DEL_SOURCE');
					}
				} .bind(this));
			}
			if(checkBoxByPass){
				checkBoxByPass.observe('click', function() {
					if(checkBoxByPass.checked){
						this.__uploadModule.addParameter('I_V_NO_CHECK','X');
					}else{
						this.__uploadModule.removeParameter('I_V_NO_CHECK');
					}
				} .bind(this));
			}
