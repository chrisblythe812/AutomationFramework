...

{
	...
	arrowClickHandler:function(evt){
		...
		var arrow = evt.element();
		arrow.up().next().toggle();	
			...
			if(arrow.hasClassName('application_down_arrow')){
				...
				if(global.liteVersion){
					arrow.innerHTML = '►';
					if(arrow.hasClassName('dm_catalog_down_lite')){
						arrow.removeClassName('dm_catalog_down_lite');
						arrow.addClassName('dm_catalog_up_lite');
					}
				}
			}else{
				arrow.removeClassName('application_verticalR_arrow');
				arrow.addClassName('application_down_arrow');
				if(global.liteVersion){
					arrow.innerHTML = '▼';
					if(arrow.hasClassName('dm_catalog_up_lite')){
						arrow.removeClassName('dm_catalog_up_lite');
						arrow.addClassName('dm_catalog_down_lite');
					}
				}
			}	
...
/* 		if(span.hasClassName('application_verticalR_arrow')){
	...
	buildDocUpload: function(doc_id,doc_type_label,required,onSuccessEvent,onFailureEvent,i){
		...
		if(doc_type_label.docAppType=='ELT'){
			...
			this.empName = empName;
			new CoversheetSD(div2,this.info.docs[i].docName,this.info.docs[i].docId,this.gcc,this.lcc,global.name,global.objectId,this.empName,this.emp,false,global.getLabel('DM_UPLOAD_TICKET'),this,i,onSuccessEvent,onFailureEvent);
			div2.hide();
		}
		else{
