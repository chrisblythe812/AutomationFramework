            ...
            for (var i = 0; i < events.length; i++) {
                ...
                if (!Object.isEmpty(eventId)) {
                    ...
                    var eventProperties = new Hash();
                        var tSheetEvent = false;
                        var isSavedOrSFAproval = false;
                    for (var j = 0; j < properties.length; j++) {
                        ...
                        var index = Object.isEmpty(properties[j]['@fieldtechname']) ? properties[j]['@fieldid'] : properties[j]['@fieldtechname'];
                            if(index == 'STATUS' && (properties[j]['@value'] == '10' || properties[j]['@value'] == '20'))
                                isSavedOrSFAproval = true;
                        ...
                        // Timesheet events -> they have the same id, so we add to their id a counter
                            if ((index == 'APPID') && (properties[j]['@value'] == this.timeSheetAppid)){
                            eventId += "_" + i;
                                tSheetEvent = true;
                            }
                        }
                        //If the event is from timesheet, is saved (status == 10) or sentForAproval (status == 20) and has button is because is a delete event in the timesheet
                        if(tSheetEvent && isSavedOrSFAproval && !Object.isEmpty(events[i].contents.yglui_str_wid_content.buttons)){
                            eventProperties.set('buttons', events[i].contents.yglui_str_wid_content.buttons);
                            //Set the status text on sent for deletion
                            for (var j = 0; j < properties.length; j++) {
                                var index = Object.isEmpty(properties[j]['@fieldtechname']) ? properties[j]['@fieldid'] : properties[j]['@fieldtechname'];
                                if(index == 'STATUS'){
                                    eventProperties.set(index, {
                                    ...
                                    properties[j]['#text'] = 'SAV_DEL';
                                    properties[j]['@value'] = '21';
                                }
                            }
                        }
                    ...
                    if (eventProperties.get('APPID').value == 'TIM_ERR' || eventProperties.get('APPID').value == 'HRS_RLZ')
                        ...
                        else if (eventProperties.get('APPID').value != this.timeSheetAppid)
                    ...
                    // Timesheet events
                        ...
                        for (var j = 0; j < events.length; j++) {
                            ...
                            event = eventHash.get(tshEventId);
                                    if (!Object.isEmpty(event)){
                                        if(Object.isEmpty(event.get('buttons')))
                                event.set('STATUS', status_code);
                                        else
                                            event.set('STATUS', 2); //Deleted events on timesheet
                                    }
                        }
                    }
                }
        ...
        // If not, we have to check its code
            ...
            if ((eventStatus == "21") || (eventStatus == "31"))
                ...
                eventHash.get('STATUS')['#text'] = global.getLabel('status_S');
                    else{//If it's a timesheet event, we need to check the event status into the record (values)
                        var event = objectToArray(json.EWS.o_field_values.yglui_str_wid_record)[eventFound].contents.yglui_str_wid_content.fields.yglui_str_wid_field;
                        for(var i = 0; i < event.length; i++){
                            var index = Object.isEmpty(event[i]['@fieldtechname']) ? event[i]['@fieldid'] : event[i]['@fieldtechname'];
                            if(index[i] == 'STATUS' && index[i]['#text'] == 'SAV_DEL')
                                eventHash.get('STATUS')['#text'] = global.getLabel('SAV_DEL');
                        }
                    }
                }
        }
