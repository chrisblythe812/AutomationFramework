...
initialize: function($super, args) {
    ...
    this.undoLabel = global.getLabel('undo').capitalize();
    this.access = 0;
    this.fristTimeComeFrom = '';
    
    //Array to save the events handler "change" for some elements in timeSheet
    this.handlersArray = [];
...
run: function($super, args) {
    ...
    //Start the observers for the input fields (if any) 
    for(var i = 0; i < this.handlersArray.length; i++)
        this.handlersArray[i].start();
    ...
    if (this.access == 0) {//First time access
        this.access++;
        this.firstTimeComeFrom = this.comeFrom;
    }
    else if (this.access == 1) {//Second time access
    ...
    if (this.comeFrom == 'inbox') {
        ...
        if (this.firstRun) {
            ...
            // Current timesheet
            this.currentTimesheet = null;
            // Hash with the events which can be saved
            this.eventsCanBeSaved = new Hash();
            // Hash which relates rowId and eventKey
            this.rowEventRel = new Hash();
            //Timesheet Appid
            this.timeSheetAppid = '';
...
onEmployeeSelected: function(args) {
    if (this.comeFrom != "inbox") {
        ...
        if (Object.isEmpty(this.currentUser) || (this.currentUser.id != args.id)) {
            ...
            this.currentDate = new Date();//Sets the current date for the new selected user
...
_getTimesheetEvents: function(json) {
    ...
    this.eventsCanBeSaved = new Hash();
    if (!Object.isEmpty(json)) {
        //Get the timesheet appid
        if(Object.jsonPathExists(json, 'EWS.o_li_incapp.yglui_str_incap2')){
            var incapp = json.EWS.o_li_incapp.yglui_str_incap2;
            for(var i = 0; i<incapp.length; i++)
                if(incapp[i]['@event'] == 'TSH'){
                    this.timeSheetAppid = incapp[i]['@appid'];
                    break;
                }
        }
        if (!Object.isEmpty(json.EWS.o_field_values)) {
            ...
            for (var i = 0; i < length; i++) {
                ...
                var statusSavedNullOrSFD = false;
                var eventStatus = '';
                var appID = '';
                for (var j = 0; j < length2; j++) {
                    ...
                    //Check the status if it's saved "10", sent for aproval "20" or approved "empty" 
                    if (subindex == 'STATUS'){
                        eventStatus = properties[j]['@value'];
                        if(eventStatus == '10' || eventStatus == '20' || Object.isEmpty(eventStatus))
                            statusSavedNullOrSFD = true;
                    }
                    //Check the appid 
                    if (subindex == 'APPID'){
                        appID = properties[j]['@value'];
                    }
                }
                if(eventStatus == '20' && appID != this.timeSheetAppid)//SFA events that doesn't belong to TimeSheet, won't be saved.
                    statusSavedNullOrSFD = false;
                var key = events[i].contents.yglui_str_wid_content['@key_str'];
                var eventId = events[i]['@rec_key'];
                eventProperties.set('ID', eventId);
                eventProperties.set('KEY', key);
                //The event will be dispensable if the event has buttons
                if(!Object.isEmpty(events[i].contents.yglui_str_wid_content.buttons)){
                    eventProperties.set('buttons', events[i].contents.yglui_str_wid_content.buttons.yglui_str_wid_button);
                    if(statusSavedNullOrSFD){
                        this.eventsCanBeSaved.set(key, {eventJson : events[i],  rowId : null, removed : false, saved : false, status : eventStatus});
                    }
                }
    ...
    if (this.inboxModeUPD) {
        ...
        td_element = new Element( 'td', {
            ...
            'class': html_class
                ...
                'title': this.addLabel,
                'class': 'applicationtimeSheet_element_centeredAdd application_main_title3 application_handCursor'
            }).insert(" + ")
        );            
        html_element.insert(td_element);
        ...
        td_element = new Element ( 'td', {
            ...
            'class': html_class
                ...
                'class': 'applicationtimeSheet_element_centeredRemove application_main_title3 application_handCursor'
            })
        );            
        html_element.insert(td_element);
    }
    ...
    if (inDateRange){
        if(!global.liteVersion)
            ...
            td_element.insert( new Element ( 'button', {
                id: 'applicationtimeSheet_addSpan_' + rowId,
                'title': this.addLabel,
                'class': 'applicationtimeSheet_element_centeredAdd application_main_title3 link'
            }).insert(" + "));
    }
    html_element.insert(td_element);
    ...
    if (inDateRange){
        if(!global.liteVersion)
            ...
            td_element.insert( new Element ( 'button', {
                id: 'applicationtimeSheet_removeSpan_' + rowId,
                'title': this.removeLabel,
                'class': 'applicationtimeSheet_element_centeredRemove application_main_title3 link'
            }));
    }
    html_element.insert(td_element);
      
    this.table.insert(html_element);
    //If the row are between the o_retro/o_future date, draws the time_type and cost_center autocompleters
    if (inDateRange) {        
        ...
        this.handlersArray.push($('applicationtimeSheet_hoursInput_' + rowId).on('change', this._checkElementToPopUp.bind(this)));
        this.handlersArray.push($('applicationtimeSheet_commentInput_' + rowId).on('change', this._checkElementToPopUp.bind(this)));
...
_enableExitPopUp: function(){
    ...
    if(index == -1){    //the popup is not enabled yet
        var info = {abortClose : global.getLabel('yes'),
            confirmClose : global.getLabel('no'),
            messageToShow : global.getLabel('AreSureTimeSheet').capitalize()
        };
        global.popUpBeforeCloseInformation.set('timeSheet', info);
...
_addRow: function(rowId) {
    ...
    //Setting the observers onChange for the input fields
    this.handlersArray.push($('applicationtimeSheet_hoursInput_' + newRowId).on('change', this._checkElementToPopUp.bind(this)));
    this.handlersArray.push($('applicationtimeSheet_commentInput_' + newRowId).on('change', this._checkElementToPopUp.bind(this)));
    //Setting the observers onChange for the autocompleter
    document.observe('EWS:autocompleterResultSelected', this._checkElementToPopUp.bind(this));
},
/**
...
_removeRow: function(rowId) {
    ...
    //Check if the row to be removed is a 'aproved event to be removed'
    var removeEvent = false;
    if($('applicationtimeSheet_removeSpan_' + rowId).innerHTML.include('x'))
        removeEvent = true;
    if(!removeEvent){//Remove the row
    ...
    // Other row
        ...
        if (indexes.length == 1) {
            var firstRowId = rowInfo[0] + "_" + stringDate + "_" + indexes[0];
                var cellDiv = $('applicationtimeSheet_removeSpan_' + firstRowId);
                if(!Object.isEmpty(cellDiv) && cellDiv.innerHTML == ' - ')
                    cellDiv.update("");
            }
        }
    }else //Remove the event
        this._removeApprovedEvent(rowId, stringDate);
},
/**
* @description Take the actions before remove the event
*/
_removeApprovedEvent: function(rowId, stringDate) {
    var date = Date.parseExact(stringDate,'yyyy-MM-dd');
    var inDateRange = date.between(this.retroDate, this.futureDate);
    //Mark this event as removed
    var eventId = this.rowEventRel.get(rowId);
    this.eventsCanBeSaved.get(eventId).removed = true; 
    var iconClass = "application_rounded_x1";
    var iconTitle = global.getLabel('SAV_DEL');
    var html_class = "";
    if (inDateRange)
    ...
    //Add the "undo" icon in place of "x" icon.
    var deletionCell = $('applicationtimeSheet_removeSpan_' + rowId).update('');
    deletionCell.writeAttribute('title', this.undoLabel);
    if(!global.liteVersion)
        deletionCell.addClassName('application_wa_undo test_icon');
    else
        deletionCell.update('&nbsp;&larr;&nbsp;')
    deletionCell.writeAttribute('id', 'applicationtimeSheet_undoSpan_' + rowId);
},
/**
* @description Undo the action to remove a aproval event
*/
_undoRow: function(rowId) {
    //Mark this event as not removed
    var eventId = this.rowEventRel.get(rowId);
    this.eventsCanBeSaved.get(eventId).removed = false;
    $('applicationtimeSheet_status_' + rowId).update('');
    //Add the "x" icon in place of "undo" icon.
    var deletionCell = $('applicationtimeSheet_undoSpan_' + rowId).update('');
    deletionCell.writeAttribute('title', global.getLabel('delete'));
    deletionCell.removeClassName('application_wa_undo test_icon');
    deletionCell.removeClassName('applicationtimeSheet_element_centeredCancel');
    deletionCell.removeClassName('application_action_link');
    deletionCell.update(" x ");
    deletionCell.writeAttribute('id', 'applicationtimeSheet_removeSpan_' + rowId);
},
/**
...
_addFilledRow: function(event) {
    ...
    */
    ...
    var iconTitle = "";
        ...
        //If the event is deleted (Saved deletion event) we put the send for deletion icon
        if(!Object.isEmpty(eventId) && this.eventsCanBeSaved.keys().include(eventId)){
            if(status == 10){
                iconClass = "application_rounded_x1";
                iconTitle = global.getLabel('SAV_DEL');
            }else if(status == 20){
                iconClass = "application_rounded_x1";
                iconTitle = global.getLabel('SENT_DELETION');
            }
        }
    do {
        var stringDate = initialDate.clone().toString(this.dateFormat);
        ...
        if(dayHours.toString().include(global.millionsSeparator))
        ...
        if(!global.liteVersion)
            ...
            //If the event is deleted we put the x1 icon
            if((status == 10) && !Object.isEmpty(event.get('buttons')))
                statusHtml.update( '&nbsp;X&nbsp;');
        }
        // Filled row or more than one row for this date -> add a new row
        if (add) 
        {
            ...
            var eventId = event.get('KEY');
            if(!Object.isEmpty(eventId) && this.eventsCanBeSaved.keys().include(eventId)){
                this.eventsCanBeSaved.get(eventId).rowId = rowId;//Store the rowId
                this.eventsCanBeSaved.get(eventId).status = status;//Store the status
                this.rowEventRel.set(rowId, eventId);//Store the relationship between the rowId and the eventId
            }
            var newRow = this.table.insertRow(position);
            newRow.id = rowId;
            newRow.className = 'applicationtimeSheet_tableRow';
            ...
            //Add icon '-' or 'x' or 'undo' to remove/undo the row
            html_class = 'applicationtimeSheet_element_remove';
            if (!this.showCC)
                html_class += "_noCC";
            td_element = new Element( 'td', {
            ...
            if (editable && inDateRange){//" - " icon
                o_element.insert(" - ");
                o_element.writeAttribute('title', this.removeLabel);
            }
            else if(this.eventsCanBeSaved.keys().include(eventId) && ((status == 30) || Object.isEmpty(status) || isNaN(status))){
                o_element.writeAttribute('title', global.getLabel('delete'));
                o_element.insert(" x ");//" x " icon
            }
            else if(this.eventsCanBeSaved.keys().include(eventId) && (status == 20)){
                o_element.insert(global.getLabel('cancel'));//" cancel " icon
                o_element.addClassName('application_action_link applicationtimeSheet_element_centeredCancel');
                o_element.writeAttribute('id', 'applicationtimeSheet_undoSpan_' + rowId);
                o_element.writeAttribute('title', global.getLabel('CANC_DEL'));
            }
            else if(this.eventsCanBeSaved.keys().include(eventId) && (status == 10)){//"undo" icon
                this.eventsCanBeSaved.get(eventId).removed =  true;
                this.eventsCanBeSaved.get(eventId).saved =  true;  
                o_element.writeAttribute('id', 'applicationtimeSheet_undoSpan_' + rowId);
                o_element.writeAttribute('title',  this.undoLabel);
                if(!global.liteVersion)
                    o_element.addClassName('application_wa_undo test_icon');
                else
                    o_element.update('&nbsp;&larr;&nbsp;')
                o_element.writeAttribute('id', 'applicationtimeSheet_undoSpan_' + rowId);
            }
            $(rowId).insert( td_element.insert(o_element ));
        ...
        // ... and not editable ones (or out of retro/future range)
            ...
            //Insert the remove icon if the status is approved/SentForDeletion or undo icon if the status is approved
            o_element =  $('applicationtimeSheet_removeSpan_' + rowId);
            if(this.eventsCanBeSaved.keys().include(eventId) && ((status == 30) || Object.isEmpty(status) || isNaN(status))){ //' x ' icon
                o_element.insert(" x ");
                o_element.writeAttribute('title', global.getLabel('delete'));
            }
            else if(this.eventsCanBeSaved.keys().include(eventId) && (status == 20)){ //' cancel ' icon
                o_element.insert(global.getLabel('cancel'));//" cancel " icon
                o_element.addClassName('application_action_link applicationtimeSheet_element_centeredCancel');
                o_element.writeAttribute('id', 'applicationtimeSheet_undoSpan_' + rowId);
                o_element.writeAttribute('title', global.getLabel('CANC_DEL'));
...
_timesheetAction: function(actionInfo) {
    ...
    var timesheetEvents = "";
    var foreign_list = "";
    for( var cont = 0; cont < this.dateCounter.keys().size(); cont++){//Loop over each day
        var dateString = this.dateCounter.keys()[cont];
        var rows = this.dateCounter.get(dateString).get('indexes');
        var length = rows.length;
        for (var i = 0; i < length; i++) {//Loop over the events for each day
            ...
            //Add into timesheetEvents the removed events
            var eventId = this.rowEventRel.get(rowId);
            if(!Object.isEmpty(eventId) && !Object.isEmpty(this.eventsCanBeSaved.get(eventId))){
                //If a event != send for deletion is removed, we send the record into the "o_event_list" (with the DEL button only).
                if(this.eventsCanBeSaved.get(eventId).removed && this.eventsCanBeSaved.get(eventId).status != '21'){
                    this.eventsCanBeSaved.get(eventId).saved = true;//Mark this event as saved
                    var evJson = this.eventsCanBeSaved.get(eventId).eventJson;
                    //Remove the submit button if it's inside the json
                    var buttons = evJson.contents.yglui_str_wid_content.buttons.yglui_str_wid_button;
                    for(var j = 0; j < buttons.length; j++){
                        if(!Object.isEmpty(buttons[j]) && buttons[j]['@action'].include('SUBMIT')){
                            delete buttons[j];
                            break;
                        }
                    }
                    ...
                    timesheetEvents += json2xml.writeXML({yglui_str_wid_record : evJson}, true);   
                }
                //If a event == send for deletion isn't removed, we send the record into the "o_event_list" (keeping all buttons).
                else if(!this.eventsCanBeSaved.get(eventId).removed && this.eventsCanBeSaved.get(eventId).status == '21'){
                    this.eventsCanBeSaved.get(eventId).saved = true;//Mark this event as saved
                    var evJson = this.eventsCanBeSaved.get(eventId).eventJson;
                    //Remove the submit button if it's inside the json
                    var buttons = evJson.contents.yglui_str_wid_content.buttons.yglui_str_wid_button;
