...
var ScmEmployeeSearch = Class.create(/** @lends ScmEmployeeSearch.prototype */{
	...
	getInputField: function(id){
		return this.parentNode.down('input#SCM_FindEmpl_' + this.ident + '_' + id);
	},
	 ...
	 * <br/>Modified for 5.4 
	 * <ul>
	 * <li>Added events to disable fields depending on user inputs in the employee search form</li>
	 * </ul>
	 * <br/>Modified for 2.2 
	 * <ul>
	...
	formEnable: function(withCompCust, field) {
		...
		fields.each(function(fieldName) {
			...
			if(!Object.isEmpty(element)) {
				...
				//since 5.4 Stop observing the event
				...
				if(fieldName === this.custCompCriteria.name) 
					...
					//since 5.4 Add event to check value of the input field after a key press
					element.observe('keyup', this.toggleInputFields.bindAsEventListener(this));					
				}
			}
		}.bind(this));
	},
	...
	searchEmployee: function(args) {
		...
		if (args.length > 1){
			//since 5.4 If there is more than 1 entry, it is a first name and last name combo
			var firstNameValue = args.find(function(field){return field.criteria == 'FIRST_NAME'}).fieldValue;
			var lastNameValue = args.find(function(field){return field.criteria == 'LAST_NAME'}).fieldValue;
			
			methodName = 'SearchByFirstAndLastName';
	        params.set('FirstName', HrwRequest.encode(firstNameValue));
			params.set('LastName', HrwRequest.encode(lastNameValue));
			params.set('ClientSkillId', this.getValues(this.custCompCriteria.id));
		}else{
			//since 2.1 Use the standard encoding
			var safeFieldValue = HrwRequest.encode(args[0].fieldValue);
			
			switch(args[0].criteria) {
				case 'EMP_ID':
    ...
    employeeSearchResultList: function(jsonListEmployees) {
        ...
        switch(employeeList.size()) {
            case 0:			
				...
				this.setValues(values, false);
				//since 5.4 Enable all search fields
				this.formEnable(this.custCompMandatory);
            ...
            case 1:
                ...
                this.setValues(values, false);
				//since 5.4 Disable fields when an employee is selected
				this.formDisable(this.custCompMandatory);
                break;
	...
	/**
	 * Allow fields to be disabled depending on where the user is typing
	 * @param {Event} event
	 * @since 5.4
	 */
	toggleInputFields: function(event){
		...
		var srcElement = event.element();
		var prefix = 'SCM_FindEmpl_' + this.ident + '_';
		var srcId = srcElement.id;
		var id = srcId.substr(prefix.length, srcId.length);
		
		//Enable all fields if the input field is empty
		if (srcElement.value === '' && (event.keyCode === 8 || event.keyCode === 46)){
			//Check if both First and Last name are empty before enabling other fields
			if ((id === 'FIRST_NAME' || id === 'LAST_NAME') && (this.getInputField('FIRST_NAME').value !== '' || this.getInputField('LAST_NAME').value !== '')) 
				return;
			...
			switch (id){
				case 'FIRST_NAME':
				case 'LAST_NAME':
					elementId = elementId.without('FIRST_NAME','LAST_NAME');
					break;
				default:
					elementId = elementId.without(id);
					break;
			}
			//Disable unnecesary fields
			elementId.each(function(element){
				if (!this.getInputField(element).disabled);
					this.formDisable(false, element);
			}, this);			
		}
	},
	
	/**
	 * Clear contents of all search fields
	 * @since 5.4
	 */
	_clearSearchForm: function(){
		this.setValues(this.resetEmployeeValues(this.getValues()));
		this.formEnable(false);
	}
});
/**
...
var ScmEmployeeSearchCustomerBased = Class.create(ScmEmployeeSearch, /** @lends ScmEmployeeSearchCustomerBased.prototype */{
    ...
    setFormInitial: function(form, disabled, customerMandatory) {
		...
		//since 5.4 Added link to clear contents
		if (!this.formDisabled) {
			var clearLink = new SCM_SingleLinkBuilder({
				label: global.getLabel('clear'),
				extraClassNames: 'SCM_FindEmpClear',
				handler: this._clearSearchForm.bind(this, true)
			});
			
			//Insert "Clear" link after the Email field
			form.childElements()[4].insert({
				after: clearLink.getLink().wrap('div', {
					'class': 'SCM_FindEmpLine'
				})
			});
		}
	...
	inputKeyPressed: function(event, accessType) {				
		...
		if (accessType === 'standard'){
			//since 5.4  Save both First and Last name if they have inputs
			var searchArgs = $A();
			var fieldId = this._lastSearchedField.identify().substr(14 + this.ident.length);
			
			searchArgs.push({criteria: fieldId, fieldValue: this._lastSearchedField.value});
			if (fieldId === 'FIRST_NAME'){
				var lastName = this.getInputField('LAST_NAME');
				if (lastName.value !== '') searchArgs.push({criteria: 'LAST_NAME', fieldValue: lastName.value});
			}else if (fieldId === 'LAST_NAME'){
				var firstName = this.getInputField('FIRST_NAME');
				if (firstName.value !== '') searchArgs.push({criteria: 'FIRST_NAME', fieldValue: firstName.value});
			}
			this.searchEmployee(searchArgs);		
		}else //For the customer field, call the method that allow to search company with a pattern
...
var ScmEmployeeSearchCompanyBased = Class.create(ScmEmployeeSearch, /** @lends ScmEmployeeSearchCompanyBased.prototype */{
	 ...
	 * @see ScmEmployeeSearch#employeeSearchResultList
	 */
    searchEmployee: function($super, args) {
		if(args[0].criteria === this.custCompCriteria.name)
        	hrwEngine.callBackend(this.parentApp, 'Admin.Bhou', $H({scAgentId: hrwEngine.scAgentId}), this.employeeSearchResultList.bind(this));
		else
			$super(args);
	},
