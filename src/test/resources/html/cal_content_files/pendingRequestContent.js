...
var PendingRequestContent = Class.create(origin, {
    ...
    */
    initialize: function($super, virtualHtml, hashOfWidgets, empId, appId, tabId, firstRun) {
        ...
        // this.virtualHtml = virtualHtml;  -- DEPRECATED --
    ...
    */
    fillPendingRequestWidget: function(appId) {
        ...
        var html = "<div id='pdcPendReq_contentPart_" + this.tabId + '_' + this.empId + "'>" +
                        "<div id='pdcPendReq_navigationPart_" + this.tabId + '_' + this.empId + "' class='pdcPendReq_navigationPart'></div>" +
                        "<div id='pdcPendReq_tableDataPart_" + this.tabId + '_' + this.empId + "'>" +
                            "<div id='pdcPendReq_legend_" + this.tabId + '_' + this.empId + "' class='pdcPendReq_legendPart'></div>" +
                            "<div id='pdcPendReq_tableContent_" + this.tabId + '_' + this.empId + "' class='pdcPendReq_tableContent'></div>" +
                            "<div id='pdcPendReq_tableContentEmpty_" + this.tabId + '_' + this.empId + "' class='pdcPendReq_emptyTableSearchData application_main_soft_text'>" + global.getLabel('noResults') + "</div>" +
                        "</div>" +
                        "<div id='pdcPendReq_emptyTableDataPart_" + this.tabId + '_' + this.empId + "' class='pdcPendReq_emptyTableDataPart application_main_soft_text'>" + global.getLabel('noPendReq') + "</div>" +
                    "</div>" +
                    "<div id='pdcPendReq_contentPartEmpty_" + this.tabId + '_' + this.empId + "' class='fieldDispTotalWidth fieldDispFloatLeft pdcPendReq_emptyTableDataPart application_main_soft_text'>" + global.getLabel('noPendReq') + "</div>";
        this._widget.setContent(html);
        //hide the empty results(search field)
        $('pdcPendReq_tableContentEmpty_' + this.tabId + '_' + this.empId).hide();
        $('pdcPendReq_emptyTableDataPart_' + this.tabId + '_' + this.empId).hide();
        $('pdcPendReq_contentPartEmpty_' + this.tabId + '_' + this.empId).hide();
    ...
    */
    callToGetPendingRequest: function(appId) {
        var widgetsAppId = '';
        var widgets = this.hashOfWidgets.keys();
        var length = widgets.length;
        for (var i = 0; i < length; i++) {
            if (widgets[i] != appId)
                widgetsAppId += "<YGLUI_STR_WID_SCREEN APPID='" + widgets[i] + "' SCREEN=''></YGLUI_STR_WID_SCREEN>";
        }
        var xml = "<EWS>" +
    ...
    processPendingRequest: function(json) {
        if (!Object.isEmpty(json.EWS.o_records)) {
            ...
            if (this.numberCurrMonth != 0) {
                ...
                this.drawNavigationLink();
                //hide the div
                $('pdcPendReq_tableDataPart_' + this.tabId + '_' + this.empId).hide();
                //show "No pending requests" message
                $('pdcPendReq_emptyTableDataPart_' + this.tabId + '_' + this.empId).show();
            }
        } else {
            //hide the div
            $('pdcPendReq_contentPart_' + this.tabId + '_' + this.empId).hide();
            //show "No pending requests" message
            $('pdcPendReq_contentPartEmpty_' + this.tabId + '_' + this.empId).show();
        }
    },
    /**
    ...
    */
    drawNavigationLink: function() {
        if (!this.dataLoaded) {
            ...
            var html = "<span id='pdcPendReq_link_Previous_" + this.tabId + '_' + this.empId + "' class='application_action_link pdcPendReq_link'>" + global.getLabel('prevMonth') + "(" + this.numberPrevMonth + ")</span>" +
                       "<span id='pdcPendReq_link_Current_" + this.tabId + '_' + this.empId + "' class='application_action_link pdcPendReq_link'>" + global.getLabel('currMonth') + "(" + this.numberCurrMonth + ")</span>" +
                       "<span id='pdcPendReq_link_Next_" + this.tabId + '_' + this.empId + "' class='application_action_link pdcPendReq_link'>" + global.getLabel('nextMonth') + "(" + this.numberNextMonth + ")</span>";
            }else{
                var html = "<button id='pdcPendReq_link_Previous_" + this.tabId + '_' + this.empId + "' title='" + global.getLabel("prevMonth") + "' class='application_action_link pdcPendReq_link link'>" + global.getLabel('prevMonth') + "(" + this.numberPrevMonth + ")</button>" +
                           "<button id='pdcPendReq_link_Current_" + this.tabId + '_' + this.empId + "' title='" + global.getLabel("currMonth") + "' class='application_action_link pdcPendReq_link link'>" + global.getLabel('currMonth') + "(" + this.numberCurrMonth + ")</button>" +
                           "<button id='pdcPendReq_link_Next_" + this.tabId + '_' + this.empId + "' title='" + global.getLabel("nextMonth") + "' class='application_action_link pdcPendReq_link link'>" + global.getLabel('nextMonth') + "(" + this.numberNextMonth + ")</button>";
            }
            //insert the links
            $('pdcPendReq_navigationPart_' + this.tabId + '_' + this.empId).update(html);
        }
        //events for each link
        $('pdcPendReq_link_Previous_' + this.tabId + '_' + this.empId).observe('click', this.updateInfo.bind(this, 'Previous'));
        $('pdcPendReq_link_Current_' + this.tabId + '_' + this.empId).observe('click', this.updateInfo.bind(this, 'Current'));
        $('pdcPendReq_link_Next_' + this.tabId + '_' + this.empId).observe('click', this.updateInfo.bind(this, 'Next'));
        //apply styles in the selected link
        $('pdcPendReq_link_' + this.actualLink + '_' + this.tabId + '_' + this.empId).removeClassName('application_action_link');
        $('pdcPendReq_link_' + this.actualLink + '_' + this.tabId + '_' + this.empId).addClassName('application_text_bolder');
        //stop the event for the selected link
        $('pdcPendReq_link_' + this.actualLink + '_' + this.tabId + '_' + this.empId).stopObserving('click');
    },
    /**
    ...
    */
    updateInfo: function(link) {
        ...
        $('pdcPendReq_link_' + this.oldLink + '_' + this.tabId + '_' + this.empId).removeClassName('application_text_bolder');
        $('pdcPendReq_link_' + this.oldLink + '_' + this.tabId + '_' + this.empId).addClassName('application_action_link');
        $('pdcPendReq_link_' + this.actualLink + '_' + this.tabId + '_' + this.empId).removeClassName('application_action_link');
        $('pdcPendReq_link_' + this.actualLink + '_' + this.tabId + '_' + this.empId).addClassName('application_text_bolder');
        //update events of links
        $('pdcPendReq_link_' + this.oldLink + '_' + this.tabId + '_' + this.empId).observe('click', this.updateInfo.bind(this, this.oldLink));
        $('pdcPendReq_link_' + this.actualLink + '_' + this.tabId + '_' + this.empId).stopObserving('click');
        ...
        if (this.DataToShowHash.keys().length != 0) {
            ...
            if ($('PendReq_searchBox'))
                $('PendReq_searchBox').value = global.labels.get('search');
            //show the div and hide the empty message
            $('pdcPendReq_tableContent_' + this.tabId + '_' + this.empId).show();
            $('pdcPendReq_legend_' + this.tabId + '_' + this.empId).show();
            $('pdcPendReq_tableContentEmpty_' + this.tabId + '_' + this.empId).hide();
            //update variable
            this.makeSearch = false;
            ...
            $('pdcPendReq_emptyTableDataPart_' + this.tabId + '_' + this.empId).hide();
            //show the div
            $('pdcPendReq_tableDataPart_' + this.tabId + '_' + this.empId).show();
            if (!this.dataLoaded) {
                //draw legend
                this.drawLegendPart();
                //draw empty table
    ...
    */
    drawLegendPart: function() {
        ...
        $('pdcPendReq_legend_' + this.tabId + '_' + this.empId).update(legendHTML);
    },
    /**
    ...
    */
    drawEmptyTable: function(_objLabels) {
        ...
        var table = "<table id='pdcPendReq_table_" + this.tabId + "_" + this.empId + "' class='sortable pdcPendReq_table'>" +
            ...
            "</tr>" +
            "</thead>" +
            "<tbody id='pdcPendReqTable_body_" + this.tabId + "_" + this.empId + "'></tbody>" +
            "</table>";
        $('pdcPendReq_tableContent_' + this.tabId + '_' + this.empId).update(table);
    },
    /**
    ...
    */
    fillTable: function() {
        ...
        $('pdcPendReqTable_body_' + this.tabId + '_' + this.empId).update(body);
        //make the sortable table with pagination
        if (!this.tableLoaded) {
            this.PRtableKit = new tableKitWithSearch($('pdcPendReq_table_' + this.tabId + '_' + this.empId), {
            ...
            this.PRtableKit.reloadTable($('pdcPendReq_table_' + this.tabId + '_' + this.empId), true);
        if (this.firstFillPD == true) {
            //create event in the table
            $('pdcPendReq_table_' + this.tabId + '_' + this.empId).observe("click", this.getDetailsInformation.bindAsEventListener(this));
    ...
    */
    startReloadTimeout: function(instance, appId) {
        ...
        var aux;
        if ($('pdcPendReq_contentPart_' + this.tabId + '_' + this.empId))
            aux = $('pdcPendReq_contentPart_' + this.tabId + '_' + this.empId).remove();
