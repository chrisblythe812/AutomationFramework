...
*/
{
    ...
    */
    run: function($super, args) {
        ...
        if (Object.isEmpty(this.appContainer)) {
    ...
    */
    _displayNewEvent: function(json) {
        ...
        if (this.advSearch && this.employeeRestriction){
            $('applicationtimeEntryScreen_employeeSelection').toggle();
        ...
        if (this.comeFromWorkbench && this.advSearch) {
            $('applicationtimeEntryScreen_advSearch').hide();
            $('applicationtimeEntryScreen_advSearch_icon').hide();
        }
        if (this.employeeRestriction) {
            $('applicationtimeEntryScreen_addSelection').observe('click', this._addMySelection.bind(this, true));
            if (this.advSearch)
                $('applicationtimeEntryScreen_advSearch').observe('click', this._advSearch.bind(this));
            ...
            if (!this.advSearch) {
                ...
                $('applicationtimeEntryScreen_employeeCount_text').update("(0)");
        }
        ...
        var textTeamCal = global.getLabel("showTeamCal");
        if (!Object.isEmpty(buttonTeamCal)) {
            buttonTeamCal.writeAttribute('title', textTeamCal);
            buttonTeamCal.update(textTeamCal);
        }
        var buttonQuotaCheck = this.fieldPanel.getButton("SCR_QUOTA_CHECK", null, 1);
        var textQuotaCheck = global.getLabel("showQuotaCheck");
        if (!Object.isEmpty(buttonQuotaCheck)) {
            buttonQuotaCheck.writeAttribute('title', textQuotaCheck);
    ...
    */
    _addMySelection: function(fireEvent) {
        ...
        if (this.advSearch)
            $('applicationtimeEntryScreen_employeeCount_text').update("(" + authorized + ")");
        if (this.comeFromWorkbench)
    ...
    */
    _toggleTeamCalendar: function(button) {
        ...
        if (buttonSendDoc) {
            buttonSendDoc.writeAttribute('title', textSendDoc);
    ...
    */
    _eventAction: function(action, okcode, type, label) {
        ...
        if (!correctemp && !$('applicationtimeEntryScreen_employeeSelectionContainer').hasClassName('applicationtimeEntryScreen_fieldError'))
            $('applicationtimeEntryScreen_employeeSelectionContainer').addClassName('applicationtimeEntryScreen_fieldError');
        if (correctfp && correctemp) {
            ...
            if ((begda != endda) && (radiogroup != this.alldfGroup) && (this.recurrenceInfo == null)) {
                ...
                if(this.eventHash.get(this.alldfGroup))
            ...
            if (!correctemp)
                $('fieldErrorMessage_' + this.appId).update(global.getLabel("selectEmp"));
        }
    },
    /**
    ...
    _showCreationStatus: function() {
        ...
        this.submitStatus.each(function(event) {
            if (!Object.isEmpty(event.value)) {
                ...
                completedHtml += "<tr>" +
                                     "<td><span class='applicationtimeEntryScreen_errorTable_employeeColumn' title='" + employeeName.gsub("'","&#39") + "'>" + employeeName + "</span></td>" +
                                     "<td><span class='applicationtimeEntryScreen_errorTable_textColumn' title='" + global.getLabel("submitDone") + "'>" + global.getLabel("submitDone") + "</span></td>" +
                                     "<td>" +
                                         "<div class='applicationtimeEntryScreen_errorTable_iconText'>S</div>" +
                                         "<div class='application_icon_green applicationtimeEntryScreen_errorTable_iconDiv' title='" + global.getLabel('completed') + "'></div>" +
                                     "</td>" +
                                 "</tr>";
            }
        }.bind(this));
        ...
        if (errorMessages > 0) {
            ...
            if ((requests - errorMessages) > 0) {
                ...
                $('applicationtimeEntryScreen_employeeSelection').update("");
            ...
            errorPopUp.create();
            TableKit.reloadTable($("applicationtimeEntryScreen_errorTable"));
            if ((requests - errorMessages) > 0)
                TableKit.reloadTable($("applicationtimeEntryScreen_completedTable"));
        }
        else {
            this.submitStatus = new Hash();
            document.fire('EWS:refreshTimesheet');
    ...
    */
    _employeeSelection: function() {
        if ($('applicationtimeEntryScreen_employeeSelectionContainer').hasClassName('applicationtimeEntryScreen_fieldError'))
            $('applicationtimeEntryScreen_employeeSelectionContainer').removeClassName('applicationtimeEntryScreen_fieldError');
        this._refreshMultiDependentFields();
    ...
    */
    _refreshEvent: function(json) {
        ...
        if (this.eventCodes.get('ABS').get('appids').include(this.fieldPanel.appId) || this.eventCodes.get('ATT').get('appids').include(this.fieldPanel.appId)) {
            if (!Object.isEmpty(global.currentSubApplication) && this.subApp != "TEAM"){
                this.subApp = "NONE";
                global.closeSubApplication();
            }
        }
        ...
        if (!buttonInfo['@action'].include(this.subApp)) {
            ...
            if(buttonInfo['@okcode'] != "INS"){
                if (this.subApp == "TEAM") {
                    ...
                    // Changing quota checker button text
                    var buttonQuotaCheck = this.fieldPanel.getButton("SCR_QUOTA_CHECK", null, 1);
                    var textQuotaCheck = global.getLabel("showQuotaCheck");
                    if (!Object.isEmpty(buttonQuotaCheck)) {
                        buttonQuotaCheck.writeAttribute('title', textQuotaCheck);
                    ...
                    var buttonSendDoc = this.fieldPanel.getButton("SCR_SEND_DOCUMENT", null, 1);
                    var textSendDoc = global.getLabel("ST_DOC");
                ...
                if(buttonInfo['@action'].include('SCR_X_'))
           ...
           '</EWS>';
            ...
            //If the event has send document capability and the docIds haven't been refreshed (and click on submit button)
            else if (this.sendDocCapability && !this.docIdrefreshed) {
                if (!this.isNew) //In case that we aren't in a new event, do the call
                    this.makeAJAXrequest($H({
                        xml: xmlin,
            ...
            else //If the event hasn't send document capability or if the docIds have been refreshed (and click on submit button)
    ...
    */ 
    _showSendDocument: function(buttonInfo) {
        ...
        if (this.subApp != "SEND") {
            ...
            // Changing send document button text
            var textHideSendDoc = global.getLabel("HIDE_DOC");
            if(!Object.isEmpty(buttonSendDoc)){
                buttonSendDoc.writeAttribute('title', textHideSendDoc);
                buttonSendDoc.update(textHideSendDoc);
            }
            this.subApp = "SEND";
            ...
            // Update the send document text button
            if(!Object.isEmpty(buttonSendDoc)){
                buttonSendDoc.writeAttribute('title', textSendDoc);
                buttonSendDoc.update(textSendDoc);
            }
    ...
    */
    _manageSendDocInfo: function (args) {
        ...
        if (!error) {
            ...
            buttonDocInfo.writeAttribute('title', this.numUploadedDocs + " " + this.labelUplDocuments);
    ...
    */
    _manageSendCovInfo: function(args) {
        ...
        buttonCovInfo.writeAttribute('title', this.numCoversheets + " " + this.labelCoversheets);
    ...
    */
    _manageDocButtons: function(json) {
        ...
        if (this.numUploadedDocs == 0 && !Object.isEmpty(buttonDocInfo)) {
            ...
            buttonDocInfo.writeAttribute('title', global.getLabel('NO_DOC_UPL'));
    ...
    */
    _displayRecurrenceWindow: function() {
        ...
        var html = "<span class='application_main_title2'>" + global.getLabel('recPattern') + "</span><br /><br />" +
                   "<table>" +
                       "<tr>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_frequencyD' type='radio' checked> " + global.getLabel('daily') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_frequencyW' type='radio'> " + global.getLabel('weekly') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_frequencyM' type='radio'> " + global.getLabel('monthly') + "</td>" +
                       ...
                       "<tr id='applicationtimeEntryScreen_rec_partternRow1'>" +
                           "<td colspan=4>" +
                               "<input id='applicationtimeEntryScreen_rec_monthlyD' type='radio' checked> " +
                       ...
                       "<tr id='applicationtimeEntryScreen_rec_partternRow2'>" +
                           "<td colspan=4>" +
                               "<input id='applicationtimeEntryScreen_rec_monthlyC' type='radio'> " +
                               "<select id='applicationtimeEntryScreen_rec_monthlyList'>" +
                               ...
                               "</select>  " +
                               "<select id='applicationtimeEntryScreen_rec_monthlyDays'>" +
                       ...
                       "<tr id='applicationtimeEntryScreen_rec_partternRow3'>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_patternMon' type='checkbox'> " + global.getLabel('monDay') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_patternTue' type='checkbox'> " + global.getLabel('tueDay') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_patternWed' type='checkbox'> " + global.getLabel('wedDay') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_patternThu' type='checkbox'> " + global.getLabel('thuDay') + "</td>" +
                       "</tr>" +
                       "<tr id='applicationtimeEntryScreen_rec_partternRow4'>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_patternFri' type='checkbox'> " + global.getLabel('friDay') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_patternSat' type='checkbox'> " + global.getLabel('satDay') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'><input id='applicationtimeEntryScreen_rec_patternSun' type='checkbox'> " + global.getLabel('sunDay') + "</td>" +
                           "<td class='applicationtimeEntryScreen_rec_partternCell'></td>" +
                       "</tr>" +
                   "</table><br />" +
                   ...
                   "<table>" +
                       ...
                       "<tr>" +
                           ...
                           "<td class='applicationtimeEntryScreen_rec_rangeRadio'><input type='radio' id='applicationtimeEntryScreen_rec_rangeA' checked></td>" +
                       ...
                       "<tr>" +
                           ...
                           "<td class='applicationtimeEntryScreen_rec_rangeRadio'><input type='radio' id='applicationtimeEntryScreen_rec_rangeD'></td>" +
        ...
        var callBack = function() {
            ...
            if (save.get('check1') && save.get('check2')) { // The data has been properly set
                $('applicationtimeEntryScreen_rec_frequencyD').stopObserving();
                $('applicationtimeEntryScreen_rec_frequencyW').stopObserving();
                $('applicationtimeEntryScreen_rec_frequencyM').stopObserving();
                $('applicationtimeEntryScreen_rec_rangeA').stopObserving();
                $('applicationtimeEntryScreen_rec_rangeD').stopObserving();
                $('applicationtimeEntryScreen_rec_monthlyD').stopObserving();
                $('applicationtimeEntryScreen_rec_monthlyC').stopObserving();
                ...
                if (!save.get('check1')) { // Number of occurrences incorrectly set
                    ...
                    $('applicationtimeEntryScreen_occErrorMessages').show();
                    $('applicationtimeEntryScreen_occErrorMessages').update(global.getLabel('occurExceeded') + ' (' + limit + ')');
                    if ((this.eventCode == 'ABS') || (this.eventCode == 'ATT'))
                        this._setDatePickersObservers(true);
                }
                else
                    $('applicationtimeEntryScreen_occErrorMessages').hide();
                if (!save.get('check2')) {                      
                    if (this.recPatNumberWrong) {  // Recurrence pattern number incorrectly set
                        $('applicationtimeEntryScreen_recPatErrorMessage').show();
                        $('applicationtimeEntryScreen_recPatErrorMessage').update(global.getLabel('recPatError'));
                    }
                    else
                        $('applicationtimeEntryScreen_recPatErrorMessage').hide();
                    
                    if (this.recRangeNumberWrong) { // Recurrence range number incorrectly set
                        $('applicationtimeEntryScreen_recRangeWrongNumber').show();
                        $('applicationtimeEntryScreen_recRangeWrongNumber').update(global.getLabel('recRangeNumberWrong'));
                    }
                    else
                        $('applicationtimeEntryScreen_recRangeWrongNumber').hide();
                    
                    if (this.recDatesOverlapping) { // End date wrongly entered
                        $('applicationtimeEntryScreen_recDatesOverlapping').update(global.getLabel('overlappingDates'));                        
                        $('applicationtimeEntryScreen_recDatesOverlapping').show();
                    }
                    else
                        $('applicationtimeEntryScreen_recDatesOverlapping').hide();
                }
                else {
                    $('applicationtimeEntryScreen_recPatErrorMessage').hide();
                    $('applicationtimeEntryScreen_recRangeWrongNumber').hide();
        ...
        $('applicationtimeEntryScreen_rec_partternRow1').hide();
        $('applicationtimeEntryScreen_rec_partternRow2').hide();
        $('applicationtimeEntryScreen_rec_partternRow3').hide();
        $('applicationtimeEntryScreen_rec_partternRow4').hide();
        // Hidding other elements
        $('applicationtimeEntryScreen_occErrorMessages').hide();
        $('applicationtimeEntryScreen_recPatErrorMessage').hide();
        $('applicationtimeEntryScreen_recDatesOverlapping').hide();
        $('applicationtimeEntryScreen_recRangeWrongNumber').hide();
        // Listening clicks for frequency radio buttons
        $('applicationtimeEntryScreen_rec_frequencyD').observe('click', this._showPattern.bind(this, 'D'));
        $('applicationtimeEntryScreen_rec_frequencyW').observe('click', this._showPattern.bind(this, 'W'));
        $('applicationtimeEntryScreen_rec_frequencyM').observe('click', this._showPattern.bind(this, 'M'));
        // Listening clicks for range radio buttons
        $('applicationtimeEntryScreen_rec_rangeA').observe('click', this._toggleRange.bind(this, 'A'));
        $('applicationtimeEntryScreen_rec_rangeD').observe('click', this._toggleRange.bind(this, 'D'));
        // Listening clicks for monthly patterns buttons (initial recurrence)
        $('applicationtimeEntryScreen_rec_monthlyD').observe('click', this._toggleMonthRec.bind(this, 'D'));
        $('applicationtimeEntryScreen_rec_monthlyC').observe('click', this._toggleMonthRec.bind(this, 'C'));
        ...
        if (!Object.isEmpty(this.recurrenceInfo)) {
            ...
            if (pattern != 'D') { // Daily is selected by default
                $('applicationtimeEntryScreen_rec_frequencyD').checked = false;
                if (pattern == 'W') // Weekly
                    $('applicationtimeEntryScreen_rec_frequencyW').checked = true;
                else // Monthly
                    $('applicationtimeEntryScreen_rec_frequencyM').checked = true;
            }
            ...
            $('applicationtimeEntryScreen_rec_patternInput').value = patternInput;
            // Days of the week ...
            var weekDays = this.recurrenceInfo.get('selected_days');
            // ... for a weekly recurrence
            if (pattern == 'W') {
                var daysOfTheWeek = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
                var length = daysOfTheWeek.length;
                for (var i = 0; i < length; i++)  {
                    if (weekDays.indexOf(daysOfTheWeek[i].toLowerCase()) < 0)
                        $('applicationtimeEntryScreen_rec_pattern' + daysOfTheWeek[i]).checked = false;
                    else
                        $('applicationtimeEntryScreen_rec_pattern' + daysOfTheWeek[i]).checked = true;
                }
            }
            // ... for a monthly recurrence
            if (pattern == 'M') {
                var options = $('applicationtimeEntryScreen_rec_monthlyDays').options;
            ...
            if (pattern == 'M') {
                ...
                if (weekDays.length == 0)
                    $('applicationtimeEntryScreen_rec_dayInput').value = dayValue;
                else {
                    var options = $('applicationtimeEntryScreen_rec_monthlyList').options;
                    var length = options.length;
                    var found = false;
                    for (var i = 0; (i < length) && !found; i++)  {
                ...
                if (weekDays.length > 0) {
                    $('applicationtimeEntryScreen_rec_monthlyD').checked = false; // Day
                    $('applicationtimeEntryScreen_rec_monthlyC').checked = true; // Custom
                }
            }
            var occurences = this.recurrenceInfo.get('nrocc');
            if (!Object.isEmpty(occurences))
                $('applicationtimeEntryScreen_rec_rangeInput').value = occurences;
            else {
                $('applicationtimeEntryScreen_rec_rangeA').checked = false; // Occurences
                $('applicationtimeEntryScreen_rec_rangeD').checked = true; // End date
            }
        }
    },
    /**
    ...
    */
    _showPattern: function(pattern) {
        switch (pattern) {
            case 'D': // Daily
                ...
                $('applicationtimeEntryScreen_rec_partternRow3').hide();
                $('applicationtimeEntryScreen_rec_partternRow4').hide();
                $('applicationtimeEntryScreen_rec_patternLabel').update(global.getLabel('days'));
                $('applicationtimeEntryScreen_rec_frequencyW').checked = false;
                $('applicationtimeEntryScreen_rec_frequencyM').checked = false;
                break;
            case 'W': // Weekly
                $('applicationtimeEntryScreen_rec_partternRow1').hide();
                $('applicationtimeEntryScreen_rec_partternRow2').hide();
                $('applicationtimeEntryScreen_rec_partternRow3').show();
                $('applicationtimeEntryScreen_rec_partternRow4').show();
                $('applicationtimeEntryScreen_rec_patternLabel').update(global.getLabel('weeks'));
                $('applicationtimeEntryScreen_rec_frequencyD').checked = false;
                $('applicationtimeEntryScreen_rec_frequencyM').checked = false;
                break;
            case 'M': // Monthly
                $('applicationtimeEntryScreen_rec_partternRow1').show();
                $('applicationtimeEntryScreen_rec_partternRow2').show();
                $('applicationtimeEntryScreen_rec_partternRow3').hide();
                $('applicationtimeEntryScreen_rec_partternRow4').hide();
                $('applicationtimeEntryScreen_rec_patternLabel').update(global.getLabel('months'));
                $('applicationtimeEntryScreen_rec_frequencyD').checked = false;
                $('applicationtimeEntryScreen_rec_frequencyW').checked = false;
                break;
        }
    ...
    /**
    *@description Toggles the range pattern
    *@param {string} pattern After X occurences ('A') or Specific day ('D')
    */
    _toggleRange: function(pattern) {
        if (pattern == 'A') // After X occurences
            $('applicationtimeEntryScreen_rec_rangeD').checked = false;
        else // Specific day
            $('applicationtimeEntryScreen_rec_rangeA').checked = false;
    },
    /**
    *@description Toggles the initial recurrence of monthly patterns
    *@param {string} pattern Specific day ('D') or Custom ('C')
    */
    _toggleMonthRec: function(pattern) {
        if (pattern == 'D') // Specific day
            $('applicationtimeEntryScreen_rec_monthlyC').checked = false;
        else // Custom
            $('applicationtimeEntryScreen_rec_monthlyD').checked = false;
    },
    /**
    ...
    */
    _saveRecurrence: function() {
        ...
        var found = false;
        var dtype = '';
        if ($('applicationtimeEntryScreen_rec_frequencyD').checked)
            dtype = 'D'; // Daily
        else if ($('applicationtimeEntryScreen_rec_frequencyW').checked)
            dtype = 'W'; // Weekly
        else if ($('applicationtimeEntryScreen_rec_frequencyM').checked)
            dtype = 'M'; // Monthly
        var nrday = '';
        var nrweek_month = '0';
        if (dtype == 'D') {
            nrday = $('applicationtimeEntryScreen_rec_patternInput').value;
            if (Object.isEmpty(nrday))
                nrday = '1';
        }
        else {
            nrweek_month = $('applicationtimeEntryScreen_rec_patternInput').value;
            if (Object.isEmpty(nrweek_month))
                nrweek_month = '1';
            if (dtype == 'M') {
                if ($('applicationtimeEntryScreen_rec_monthlyD').checked) { // Day selected
                    nrday = $('applicationtimeEntryScreen_rec_dayInput').value;
                    if (Object.isEmpty(nrday))
                        nrday = '1';
                }
                else // Custom selected
                    nrday = $('applicationtimeEntryScreen_rec_monthlyList').value;
            }
        }
        ...
        if (dtype == 'W') {
            ...
            var length = daysOfTheWeek.length;
            for (var i = 0; i < length; i++)  {
                if ($('applicationtimeEntryScreen_rec_pattern' + daysOfTheWeek[i]).checked)
                    selected_days.push(daysOfTheWeek[i].toLowerCase());
            }
            if (selected_days.length == 0)
                selected_days.push('mon');
        }
        if (dtype == 'M') {
            if ($('applicationtimeEntryScreen_rec_monthlyC').checked) // Custom selected
                selected_days.push($('applicationtimeEntryScreen_rec_monthlyDays').value);
        }
        ...
        var nrocc = '';
        var endda = '';
        if ($('applicationtimeEntryScreen_rec_rangeA').checked) {
            nrocc = $('applicationtimeEntryScreen_rec_rangeInput').value;
    ...
    */
    _displayRecurrentEvent: function() {
        ...
        if (this.sendDocCapability && (this.eventCodes.get('ABS').get('appids').include(this.fieldPanel.appId) || this.eventCodes.get('ATT').get('appids').include(this.fieldPanel.appId)))
            this._manageDocButtons(this.fieldPanel.json);       
        this._checkSelectedEmployees(false);//Disable or not the quotaChecker button
        $(this.appId + '_1_0_REC_TEXT').addClassName('fieldPanelVisualDepRecurrence');
        $('REC_TEXT_' + this.appId + '_1_0').addClassName('fieldPanelVisualDepRecurrenceContainer');
        if ((this.eventCode == 'ABS') || (this.eventCode == 'ATT'))
            this._setDatePickersObservers(true);
    },
    /**
    ...
    */
    _toggleSelection: function() {
        $('applicationtimeEntryScreen_employeeSelection').toggle();
        $('applicationtimeEntryScreen_addSelection').toggle();
        if (!this.comeFromWorkbench) {
            $('applicationtimeEntryScreen_advSearch').toggle();
            $('applicationtimeEntryScreen_advSearch_icon').toggle();
        }
    },
    /**
    ...
    */
    _employeeSelected: function() {
        ...
        if (this.advSearch)
            $('applicationtimeEntryScreen_employeeCount_text').update("(" + empCounter + searchMark + ")");
        if ($('applicationtimeEntryScreen_employeeSelectionContainer').hasClassName('applicationtimeEntryScreen_fieldError'))
            $('applicationtimeEntryScreen_employeeSelectionContainer').removeClassName('applicationtimeEntryScreen_fieldError');
        // Refreshing fields depending on employee selections
    ...
    */
    _setStoredSearches: function(json, ID) {
        ...
        $('applicationtimeEntryScreen_employeeCount_text').observe('click', this._toggleSelection.bind(this));
        $('applicationtimeEntryScreen_employeeCount_text').addClassName('application_action_link');
    ...
    */
    _refreshMultiselect: function(args) {
        ...
        if (!markAsSelected) {
            ...
            $('applicationtimeEntryScreen_employeeCount_text').update("(" + this.multiSelect.selectedElements.length + ")");
        }
        this._refreshMultiDependentFields(true);
    },
    /**
    ...
    */
    ...
    _correctDateSelected: function( args) {
        ...
        if (changed || ((!this.radio_changed || (!this.isNew && !this.radio_changed)) && (!Object.isEmpty(this.isNew) && !Object.isEmpty(this.radio_changed)))) {
            var begda = this.eventHash.get('BEGDA')['@value'];
            var endda = this.eventHash.get('ENDDA')['@value'];
