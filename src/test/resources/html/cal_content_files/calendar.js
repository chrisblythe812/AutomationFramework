...
*/
{
    ...
    */
    initialize: function($super, options) {
        $super(options);
        // Caching the last call performed for each employee (needed for unselections)
        this._lastCachedCalls = new Hash();
        // Bindings
    ...
    */
    renderSelectionDiv: function() {
        ...
        if (Prototype.Browser.IE) {
            // User Agent properties needed (see below)
            var msie = parseInt(navigator.userAgent.substring(navigator.userAgent.indexOf("MSIE") + 5));
            var trident = parseInt(navigator.userAgent.substring(navigator.userAgent.indexOf("Trident") + 8));
            // In compatibility view. IE9 reports itself as IE7 (this is done for compatibility).
            // An incremented Trident token, from 4.0 to 5.0, allows us to distinguish between IE8/9
            // running in Compatibility View.
            var isIE6 = (msie == 6);
            var isIE7 = (msie == 7);
            if ((isIE6 || isIE7) && (trident != 5))
                dayWidth += 1;
        ...
        for (var i = 0; i < datesHeadersLength; i++) {
            for (var j = 0; j < 7; j++, k++) {
                ...
                var width = dayWidth;
                // For each row, we set the last selection cell smaller (Style improvement)
                if ((j + 1) == 7)
                    width -= 2;
    ...
    */
    onEmployeeUnselected: function(event) {
        var employee = getArgs(event).id;
        var cachedService = this._lastCachedCalls.get(employee);
        if (!Object.isEmpty(cachedService))
            this.serviceGetEvents(employee, cachedService.json, 'unselect' + cachedService.ajaxID);
    },
    /**
    ...
    */
    ...
    /**
    * It handles the employeeSelected event getting needed data to show this new employee's events on the calendar
    * @param event the info about this employee selection
    */
    onEmployeeSelected: function(event) {
        ...
        this.ajaxRequestArguments.set(ajaxID, {
    ...
    */
    contextualBalloon: function(json, ID) {
        ...
        if (!Object.isEmpty(json.EWS.o_actions)) {
            ...
            if(payroll_run)
                actionsList.insert(new Element("span", {'class': 'test_text'}).insert('<BR>' + global.getLabel('actionHide') + (' ') + global.getLabel('duePayroll')));
    ...
    */
    serviceGetEvents: function(employeeID, json, ajaxID) {
        //Get the timeSheet appid (to not hardcode it) and delete the incapp entry for it
        ...
        json.EWS.o_li_incapp.yglui_str_incap2 =  incapp.compact();

        this.json = json;
        var unselect = ajaxID.startsWith('unselect');
        if (!unselect) {
            // Storing as the last call performed for this employee (unselections cache)
            this._lastCachedCalls.set(employeeID, {ajaxID: ajaxID, json: json});
        }
        if (!this.filterCreated) {
            var incapp = json.EWS.o_li_incapp.yglui_str_incap2;
            this.createFilterPanel(this.filterElement, this.appName, incapp);
            $('applicationCAL_filterDiv').update(global.getLabel('filterOptions')).observe('click', function() {
                this.filterElement.toggle();
                if (Prototype.Browser.Gecko || Prototype.Browser.WebKit) //If FF or Chrome, recalculate the selectionDiv position
                    this.selectionDiv.clonePosition(this.tbody);
            }.bind(this));
        ...
        var cachedService = this._lastCachedCalls.get(employeeID);
        if (!Object.isEmpty(cachedService)) {
            var selectionAjaxID = ajaxID.gsub('unselect', '');
            var ajaxBounds = this.ajaxRequestArguments.get(selectionAjaxID).bounds;
