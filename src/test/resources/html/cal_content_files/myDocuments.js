var MyDocuments = new Class.create(Application, {
	...
	listMode: null,
	__DMLightMode: null,
	_scrollTarget: null,// The element on which we will listen for scroll elements	
    ...
    initialize: function($super, args) {
		...
		this.intanceCF=null;
		this._scrollTarget =  args.scrollTarget;
    },
    ...
    clearFilterHandler: function() {
		...
		if (this.intanceCF){
			$('myDocuments_coverFlowContainer').update('');
			delete this.intanceCF;
		}
    ...
    filterDocumentsList: function() {
        ...
        documents = documents.sort(function(o1, o2) {
            return (prepareTextToEdit(o1['@doc_name']) > prepareTextToEdit(o2['@doc_name']));
        });
        var date = '';
        for (var i = 0; i < documents.length; i++) {

            if (this.filterValues.docType && documents[i] && prepareTextToEdit(documents[i]['@doc_type'])) {
                if (!prepareTextToEdit(documents[i]['@doc_type']).toLowerCase().include(this.filterValues.docType.toLowerCase())) {
                    delete documents[i];
                }else{
					if(this.filterValues.docType.toLowerCase()!=prepareTextToEdit(documents[i]['@doc_type']).toLowerCase()){
						delete documents[i];
					}
				}
            }
            ...
            if (this.filterValues.search && documents[i]) {
                ...
                if ((prepareTextToEdit(documents[i]['@doc_name']) && !prepareTextToEdit(documents[i]['@doc_name']).toLowerCase().include(this.filterValues.search))
				&& (prepareTextToEdit(documents[i]['@doc_type']) && !prepareTextToEdit(documents[i]['@doc_type']).toLowerCase().include(this.filterValues.search))
    ...
    buildHeader: function() {
        ...
        var myDocuments_SelectViewContainer = new Element('div',{
                         ...
                         'style':'text-align:center;float:right;margin-right:16px;width:'+((this.__DMLightMode)? '27%': '35%')
        });
    ...
    highlightRow: function (event) {
		var activeDocId = this.intanceCF._activeItem.content.id.replace('docFlow_', '');
		var activeElm = $('myDocuments_TrDocument' + activeDocId);
		
		//select all rows from table with class 'table_highlight_selected'
		var selectedRows = $('myDocuments_ListContainer').select('tr.table_highlight_selected');
		//loop through the results removing the classname 'table_highlight_selected'
		selectedRows.each(function(row){
			row.removeClassName('table_highlight_selected');
		},this);
		activeElm.addClassName('table_highlight_selected');	
		
		return;
		
    },
	
	buildCoverFlow: function(documentsList){
		var globalCap = new Element('div', { 'class': 'globalCaption' });
		var scroll = new Element('div', { 'class': 'scrollbar' });
		var slider = new Element('div', { 'class': 'slider' });
		var position = new Element('div', {'class':'position'});
		var elem1 = new Element('div', { 'class': 'flow' });
		this.contentFlowDiv = new Element('div',{'id': 'contentFlowDiv','class':'whoiswho_contentFlowDiv ContentFlow ContentFlowAddOn_fancyScrollbar'});
		var loadindicator = new Element('div',{'class':'loadIndicator'});
		...
		for(var x = 0; x<=this.docList.size()-1;x++){
			if (documents[x]){			
				elem2 = new Element('div', { 'class': 'item' });
				elem1.insert(elem2);
				imageElm = new Element('img', { 'id': 'docFlow_'+documents[x]['@doc_id'], 'class': 'content', 'src': this.buildURL(this.getImageUrl(documents[x]['@doc_id'])) });
				elem2.insert(imageElm);
			}
		}
		
		loadindicator.insert(indicator);
		slider.insert(position);
		scroll.insert(slider);
		this.contentFlowDiv.insert(loadindicator);		
		this.contentFlowDiv.insert(elem1);
		this.contentFlowDiv.insert(globalCap);
		this.contentFlowDiv.insert(scroll);
		
		$('myDocuments_coverFlowContainer').insert(this.contentFlowDiv);
		if (this.view == this.viewValues.CoverFlow)
			$('myDocuments_coverFlowContainer').show();
		this.intanceCF = new ContentFlow(this.contentFlowDiv.id, { reflectionColor: "#000000" });	
		
		var highlight = this.highlightRow.bind(this);
		
		if (document.attachEvent){ //if IE (and Opera depending on user setting)
			this.contentFlowDiv.attachEvent("onmousewheel", highlight);
			this.contentFlowDiv.attachEvent("onDOMMouseScroll", highlight);
			this.contentFlowDiv.addEventListener('onclick', highlight, false);
		}
		if (window.addEventListener){ 
			this.contentFlowDiv.addEventListener('mousewheel', highlight, false);
			this.contentFlowDiv.addEventListener('DOMMouseScroll', highlight, false);
			this.contentFlowDiv.addEventListener('click', highlight, false);
		}
		
	},	
	
	getImageUrl: function(docId){
		var xmlin = ""
		+ "<EWS>"
		...
		+ "<DEL/><GCC/><LCC/>"
		+ "<PARAM>"
		+ "<I_V_CONTENT_ID>" + docId + "</I_V_CONTENT_ID>"
		+ "</PARAM>"
		+ "</EWS>";
		return xmlin;
	},	
    ...
    buildDocumentsTable: function(documents) {
        ...
        if ((this.view == this.viewValues.List) || (this.view == this.viewValues.CoverFlow)) {
            ...
            documents.each(function(document) {
				...
				if (document) {
                    html += ''
					+ '<tr id="myDocuments_TrDocument' + document['@doc_id'] + '" docid="' + document['@doc_id'] +'" style="cursor:pointer;">'
					+ '				<td><div><input id="myDocuments_check' + document['@doc_id'] + '" type="checkbox" />' + underlineSearch((prepareTextToEdit(document['@doc_name']) || ''), this.filterValues.search, 'applicationInbox_textMatch') + '</div></td>'
					+ '             <td><div>' + underlineSearch((auxText || ''), this.filterValues.search, 'applicationInbox_textMatch') + '</div></td>'
					+ '             <td><div>' + underlineSearch((prepareTextToEdit(document['@doc_type']) || ''), this.filterValues.search, 'applicationInbox_textMatch') + '</div></td>'
            ...
            documents.each(function(document) {
                ...
                if (document) {
					...
					+ '		<img title="' + document['@doc_name'] + '" id="myDocuments_ThumbnailGrid' + document['@doc_id'] + '" style="border:0;width:144px;height:192px;margin-top:1px;cursor:pointer;" src="' + this.buildURL(xmlin) + '&nocach=' + Math.floor(Math.random() * 100001) + '" /><br/>' + underlineSearch((prepareTextToEdit(document['@doc_name']) || ''), this.filterValues.search, 'applicationInbox_textMatch')
    ...
    registerEvents: function(documents) {
		...
		this.docList  = objectToArray(documents);
		this.docArray = $A();
		this.filteredList = $A();
		
		for(var x = 0; x<=this.docList.size()-1;x++){
			if (documents[x]){			
				this.docArray.push(this.docList[x]['@doc_id']);
			}else{
				if (this.intanceCF){
					this.intanceCF.moveToIndex(x);
					this.intanceCF.rmItem();
				}
			}
		}
        ...
        documents.each(function(document) {
            if (document) {
                if ($('myDocuments_TrDocument' + document['@doc_id'])) {
                    ...
                    if (this.view == this.viewValues.CoverFlow) {
						$('myDocuments_TrDocument' + document['@doc_id']).observe('click',function(e){
							var docId = e.element().up().up().readAttribute('docid');
							this.intanceCF.moveToIndex(this.docArray.indexOf(docId));
							if(this.__prevSelectedElm){
								var elm = this.__prevSelectedElm;
								elm.removeClassName('table_highlight_selected');
							}
							e.element().up().up().addClassName('table_highlight_selected');							
							this.__prevSelectedElm = e.element().up().up();
	...
	buildDocHash:function(nodeId, data){
		...
		}).insert(prepareTextToEdit(data['@doc_name']));
